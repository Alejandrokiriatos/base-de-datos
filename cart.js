...
async function checkout() {
  ...
  document.getElementById('checkout-form').addEventListener('submit', async function(e) {
    ...
    const formData = {
      ...
      district: document.getElementById('district').value === 'otro' ? 
               document.getElementById('custom-district').value : 
               document.getElementById('district').value,
      address: document.getElementById('address').value,
      total_amount: cart.reduce((total, item) => total + item.price * (item.quantity || 1), 0),
      items: cart.map(item => ({
        product_id: item.id,
...
document.addEventListener('DOMContentLoaded', () => {
  const productImages = document.querySelectorAll('.producto-imagen');
